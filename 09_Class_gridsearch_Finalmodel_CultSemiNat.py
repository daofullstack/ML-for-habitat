#!/usr/bin/env/python3
'''
Description: A script for machine learning based alogorithm for Separating Cultivated and Semi_natural class
...
Cal_vector = 'Grassland_training_27700_buff30mn_id.shp'
Val_vector = 'Cultivated_SemiNat_Validation_Plot200_30mbuff_id.shp'

OutDir = InDir + 'Model_S2medTerrain_feat8'
...
#Training data
EOData= pd.read_csv(InDir + 'Cultivated_SemiNat_VI2016to2019_S2Terrain_plot600.csv', ',', index_col=False)
...
print(EOData.head())
EOData = EOData[EOData.FebtoAprS2REP2016to2019med != -999]
print(EOData.shape)

EOData = EOData[~EOData.isin([np.nan, np.inf, -np.inf, -999]).any(1)]
print(EOData.shape)

GrassTypeCal = gpd.read_file(Cal_vector)
GrassTypeCal = GrassTypeCal.drop(columns='geometry')
GrassTypeCal = GrassTypeCal[['id', 'ID_1']]
# 
EOData = pd.merge(EOData, GrassTypeCal, how='inner', on=['ID_1'])
EOData["id"] = EOData["id"].astype(int)
...
'''
#Testing data
TestData_all = pd.read_csv(ValDir + 'Cultivated_SemiNat_VI2016to2019_S2Terrain_Valplot200.csv', ',', index_col=False)
...
TestData_all = TestData_all[TestData_all.FebtoAprS2REP2016to2019med != -999]

print(TestData_all.shape)

TestData_all = TestData_all[~TestData_all.isin([np.nan, np.inf, -np.inf, -999]).any(1)]
print(TestData_all.shape)

GrassTypeVal = gpd.read_file(Val_vector)
GrassTypeVal = GrassTypeVal.drop(columns='geometry')
GrassTypeVal = GrassTypeVal[['id', 'ID_1']]
# 
TestData_all = pd.merge(TestData_all, GrassTypeVal, how='inner', on=['ID_1'])
TestData_all["id"] = TestData_all["id"].astype(int)
